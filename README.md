...
- **Quiz Generation**: Intelligent quiz creation from fisioterapia documentation
- **Medical Entity Extraction**: Anatomical structures, pathological conditions, treatments
- **Clinical Reasoning**: Evidence-based question generation and validation
- **Italian Language**: Full support for Italian medical education
- **Hybrid Search**: Semantic similarity + medical knowledge graph traversal
- **Educational Analytics**: Progress tracking and learning optimization# Medical RAG with Knowledge Graph for Fisioterapia Education

Intelligent quiz generation system for physiotherapy education using hybrid RAG architecture. Combines vector search with knowledge graphs to create contextually relevant and pedagogically sound educational assessments from medical documentation.

Built with:

- Pydantic AI for the Medical AI Agent Framework
- Graphiti for the Medical Knowledge Graph
- Postgres with PGVector for the Vector Database
- Neo4j for the Knowledge Graph Engine (Graphiti connects to this)
- FastAPI for the Medical Agent API
- Italian Language Support for Educational Content

## Overview

This system includes four main components:

1. **Medical Document Ingestion Pipeline**: Processes physiotherapy documentation using semantic chunking and builds both vector embeddings and medical knowledge graph relationships
2. **Medical AI Agent Interface**: A conversational agent powered by Pydantic AI that can search across medical content and generate educational quizzes
3. **Quiz Generation System**: Creates intelligent quizzes based on medical documentation with clinical reasoning and evidence-based validation
4. **Streaming API**: FastAPI backend with real-time streaming responses and comprehensive medical search capabilities
