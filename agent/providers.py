...
def get_llm_model_name(model_choice: Optional[str] = None) -> str:
    """
    Get LLM model name based on environment variables.
    
    Args:
        model_choice: Optional override for model choice
    
    Returns:
        The name of the LLM model as a string.
    """
    return model_choice or os.getenv('LLM_CHOICE', 'gpt-4-turbo-preview')
...
def get_ingestion_model_name() -> str:
    """
    Get ingestion-specific LLM model name (can be faster/cheaper than main model).
    
    Returns:
        Configured model name for ingestion tasks
    """
    ...
    return get_llm_model_name(model_choice=ingestion_choice)
